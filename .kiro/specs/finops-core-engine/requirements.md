...
The FinOps Core Engine is the deterministic, testable foundation of the FRT FinOps Platform. It delivers the framework and contracts for ingesting cloud billing data, normalizing it to the FOCUS 1.4 standard, enforcing data quality with a fail-closed policy, and allocating cost to teams through a 100% rule-based (non-AI) engine.
...
- **ETL_Normalizer**: The component that maps raw vendor billing records to the FOCUS 1.4 schema.
- **FOCUS_Record**: A billing record conforming to the FOCUS 1.4 standard as defined by the `FOCUS_NORMALIZED` model (BilledCost, EffectiveCost, ProviderName, SubAccountId, ServiceCategory, ServiceName, ResourceId, ChargeCategory, BillingCurrency, Tags, Region, BillingPeriod).
...
- **Allocated_Record**: A FOCUS_Record enriched with TeamID, BusinessUnit, Environment, the applied Allocation_Rule identifier, split method, and split ratio.
...
- **Billing_Period**: The period identifier for a set of billing records, expressed per the FOCUS data model.
...
### Requirement 1: FOCUS 1.4 Data Model

**User Story:** As a platform engineer, I want a FOCUS 1.4-compliant data model, so that billing data from any provider is represented in one canonical, standards-aligned schema.

#### Acceptance Criteria

1. THE Core_Engine SHALL define a FOCUS_Record type containing the fields BilledCost, EffectiveCost, ProviderName, SubAccountId, ServiceCategory, ServiceName, ResourceId, ResourceName, ChargeCategory, BillingCurrency, Tags, Region, and Billing_Period.
...
4. THE Core_Engine SHALL define an Allocated_Record type that contains the FOCUS_Record billing fields plus TeamID, BusinessUnit, Environment, allocation_rule_id, split_method, and split_ratio.
5. IF a FOCUS_Record is constructed with a missing required FOCUS field, THEN THE Core_Engine SHALL reject the record with a validation error identifying the missing field.
6. WHERE a ChargeCategory value is supplied, THE Core_Engine SHALL constrain the value to the FOCUS-defined set including Usage, Tax, and Credit.
...
1. WHEN the Ingestion_Pipeline invokes the ETL_Normalizer with raw records from a Connector_Adapter, THE ETL_Normalizer SHALL produce FOCUS_Record values conforming to the FOCUS 1.4 data model.
...
7. THE Allocation_Engine SHALL stamp each Allocated_Record with TeamID, BusinessUnit, Environment, the applied allocation_rule_id, split_method, and split_ratio.
