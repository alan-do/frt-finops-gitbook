...
| :--- | :--- | :--- |
| billing_period | string | YYYY-MM · partition key, derived from BillingPeriodStart |
| charge_period_start | timestamp | FOCUS: ChargePeriodStart (charge-level granularity) |
...
| service_category | string | FOCUS: ServiceCategory (canonical service domain) |
| service_subcategory | string | FOCUS: ServiceSubcategory (raw FPT/provider group) |
...
| service_subcategory | string | Carried from the FOCUS record |
...
| environment | string | dev \| staging \| production |
| allocated_method_id | string | FOCUS: AllocatedMethodId · applied rule id / rule type |
| allocated_method_details | string | FOCUS: AllocatedMethodDetails · split detail (internal split_ratio maps here, largest-remainder) |
| allocated_resource_id | string | FOCUS: AllocatedResourceId · resource the cost is allocated from |
| allocated_tags | map | FOCUS: AllocatedTags · tags that drove the allocation |
...
These tables store high-volume billing records and are optimized for aggregation queries. All schemas are aligned to the official **FOCUS** column standard, targeting **FOCUS 1.2** as the mandatory baseline (most provider exports — AWS/Azure/GCP — are at v1.2); FOCUS 1.4-only columns are optional/nullable.
...
- `ALLOCATED_COST` stores `allocated_method_details` (carrying the internal split ratio) for shared-cost rows, enabling full auditability of proportional distributions.
...
The `FOCUS_NORMALIZED` table is the canonical source of truth. Below is the mapping from vendor-specific columns to the official FOCUS standard (FOCUS 1.2 mandatory baseline; FPT Cloud has no native FOCUS export so its connector transforms 100% and is validated by a FOCUS conformance check):
