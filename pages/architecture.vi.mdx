...
C4Context
    ...
    Person(user, "FinOps User", "Engineers, Finance, CTO")
    
    System_Boundary(finops, "Enterprise FinOps Platform") {
        System(platform, "Core Platform", "Ingests, Normalizes, Allocates, Analyzes")
    }
    
    System_Ext(aws, "AWS CUR", "Billing Data")
    System_Ext(azure, "Azure Billing", "Billing Data")
    System_Ext(gcp, "GCP Billing", "Billing Data")
    System_Ext(k8s, "Kubernetes", "Usage Data")
    System_Ext(erp, "ERP / CMDB", "Org & Financial Data")
    System_Ext(llm, "LLM Providers", "OpenAI / Claude / Gemini")
    
    Rel(user, platform, "Views dashboards, approves Agent actions")
    Rel(platform, aws, "Pulls")
    Rel(platform, azure, "Pulls")
    Rel(platform, gcp, "Pulls")
    Rel(platform, k8s, "Pulls")
